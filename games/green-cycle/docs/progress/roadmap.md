...
### 任务清单
- [x] **合成 UI**：多塔选中交互（框选/Shift 多选）、配方提示、合成确认
- [x] **合成逻辑**：校验配方、消耗材料塔、生成产物塔、返还差价
- [x] **配方数据**：`data/recipes.ts` 已有 3 配方，已确认可用

### 涉及文件
- `game/Game.ts` — 合成交互处理
- `utils/RecipeUtil.ts` — 新增配方匹配与合成执行
- `data/recipes.ts` — 配方数据
- `index.html` — 合成 UI 元素
...
### 任务清单
- [x] **存档读写**：`utils/SaveManager.ts` 实现 localStorage 读写与版本校验
- [x] **难度解锁**：通关后解锁更高难度与无尽模式
- [x] **无尽模式**：实现 50 波后循环刷怪与强度缩放
- [x] **排行榜**：记录历史最高分（PF 分/波次/难度）
...
### 任务清单
- [x] **音效细化**：为技能释放、暴击、合成、Boss 出场等添加专属音效
- [x] **UI 美化**：塔面板分类色/图标、技能按钮交互、波次提示动画
- [x] **Quadtree 索敌优化**：`TowerAISystem` 接入 Quadtree 替换线性遍历
- [x] **离屏 canvas 缓存**：地图静态层用离屏 canvas 缓存，减少每帧重绘
