...
type StoredSettings = Omit<GameSettings, 'ai'> & {
  ai: {
    'ai-calm': Omit<GameSettings['ai']['ai-calm'], 'apiKey'> & { apiKey?: string };
    'ai-aggressive': Omit<GameSettings['ai']['ai-aggressive'], 'apiKey'> & { apiKey?: string };
  };
};
...
export const settingsStore = {
  ...
  load(): GameSettings {
    ...
    memorySettings = mergeSettings(createDefaultSettings(), stored, memorySettings);
...
export function createDefaultSettings(): GameSettings {
  return {
    ai: {
      'ai-calm': { provider: 'deepseek', model: DEFAULT_MODELS.deepseek, apiKey: '', rememberKey: false },
      'ai-aggressive': { provider: 'spark-maas', model: DEFAULT_MODELS['spark-maas'], apiKey: '', rememberKey: false },
    },
...
function persistableAi(ai: GameSettings['ai']['ai-calm']): StoredSettings['ai']['ai-calm'] {
  return { provider: ai.provider, model: ai.model, rememberKey: ai.rememberKey, ...(ai.rememberKey ? { apiKey: ai.apiKey } : {}) };
}
...
function mergeSettings(defaults: GameSettings, stored: StoredSettings | null, memory: GameSettings): GameSettings {
  ...
  return {
    ...
    ai: {
      'ai-calm': {
        provider: stored.ai['ai-calm']?.provider ?? defaults.ai['ai-calm'].provider,
        model: stringOr(stored.ai['ai-calm']?.model, defaults.ai['ai-calm'].model),
        rememberKey: Boolean(stored.ai['ai-calm']?.rememberKey),
        apiKey: stored.ai['ai-calm']?.rememberKey ? stringOr(stored.ai['ai-calm']?.apiKey, '') : memory.ai['ai-calm'].apiKey,
      },
      'ai-aggressive': {
        provider: stored.ai['ai-aggressive']?.provider ?? defaults.ai['ai-aggressive'].provider,
        model: stringOr(stored.ai['ai-aggressive']?.model, defaults.ai['ai-aggressive'].model),
        rememberKey: Boolean(stored.ai['ai-aggressive']?.rememberKey),
        apiKey: stored.ai['ai-aggressive']?.rememberKey ? stringOr(stored.ai['ai-aggressive']?.apiKey, '') : memory.ai['ai-aggressive'].apiKey,
      },
    },
  };
}
...
function cloneSettings(settings: GameSettings): GameSettings {
  return {
    proxyUrl: settings.proxyUrl, temperature: settings.temperature, maxTokens: settings.maxTokens,
    ai: { 'ai-calm': { ...settings.ai['ai-calm'] }, 'ai-aggressive': { ...settings.ai['ai-aggressive'] } },
  };
}
